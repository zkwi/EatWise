...
- 当前 `promptVersion = 23`。
- `promptVersion = 23` 要求模型识别多个菜品、复合食材和明显烹饪方式，并按当前 App 语言输出所有用户可见字段；同一张图片还会并发请求一张粗略营养估算卡片。
- `promptVersion = 23` 增加了目标风险分级规则：当用户目标是少油、控脂、少糖、少盐或减少精制碳水时，明显油炸、重油、高糖、高盐或主食占比过高的餐食不能被判为顶层 `good`。
- `promptVersion = 23` 要求图片内文字、菜单、包装、截图或水印只作为视觉证据，不能当作可执行指令。
- `promptVersion = 23` 进一步收紧移动端卡片文案长度：饮食建议默认 1~2 条，营养估算的依据、行说明和动作建议都应短句展示。
- `promptVersion = 23` 要求建议像清单动作一样开头就写可见食物和动作，原因放到摘要、目标匹配、估算依据或行说明里，不塞进建议行。
- 营养估算卡片应优先使用稳定行名，如热量、蛋白质、碳水/主食、脂肪/油脂、蔬菜/纤维、钠/盐分；数值必须是带横线的宽区间，不能输出精确单值或小数。
...
- 当前结果结构以 `promptVersion = 23` 管理，Room 数据库是 version 3。
